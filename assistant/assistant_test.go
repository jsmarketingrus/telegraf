...
func (ast *Assistant) getPluginID(name string) string {
	...
	allRes := ast.handleRequest(&getAllReq)
...
func TestAssistant_MultiIDIntegrationTest(t *testing.T) {
	...
	allRunningRes := ast.handleRequest(&getAllRunningReq)
	...
	startMemcachedRes := ast.handleRequest(&startMemcachedReq)
	...
	stopMemcachedRes := ast.handleRequest(&stopMemcachedReq)
	...
	startIDBRes := ast.handleRequest(&startIDBReq)
	...
	stopIDBRes := ast.handleRequest(&stopIDBReq)
	...
	startIDBRes2 := ast.handleRequest(&startIDBReq2)
	...
	getIDBRes3 := ast.handleRequest(&getIDBReq3)
	...
	getCPURes3 := ast.handleRequest(&getCPUReq3)
